...
public class DBTermineJDBCDao implements DBTermineDao {
	...
	public void insertTermine(DBTermine p) {
		try {
			...
			ps.executeUpdate(); /* Die Insertmethoden, um Termine in die Datenbank einfügen zu können */
	...
	public List<DBTermine> findTermineByName(DBTermine p) {
		try {
			...
			while (rs.next()) { /* Die Methode, um einen bestimmten Termin beim Namen finden zu können */
				p = new DBTermine();
	...
	public void deleteTermine(DBTermine p) {
		try {
			...
			PreparedStatement ps = con.prepareStatement(sql);
			ps.setInt(1, p.getId());
			ps.executeUpdate(); /* Mit dieser Methode kann man Termine löschen */
		} catch (SQLException ex) {
			throw new RuntimeException(ex);
		} finally {
